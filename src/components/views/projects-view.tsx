/**
 * Shared projects index renderer (locale-aware).
 */
...
async function ProjectsContent({ locale }: ProjectsViewProps) {
      ...
      <div className="page-projects space-y-10">
        <div className="text-center py-8 md:py-12">
          ...
          <div className="mt-4 flex items-center justify-center gap-4">
            ...
            {projectsConfig.tag && (
              <span className="text-sm text-muted-foreground">
                · {getLabel(config, "projects.tag")}{" "}
                <code className="bg-muted px-1.5 py-0.5 rounded text-xs">{projectsConfig.tag}</code>
              </span>
            )}
          </div>
        </div>
  ...
  const projects = getProjectsMeta(locale);
  return (
    <div className="page-projects space-y-10">
      <div className="text-center py-8 md:py-12">
        <div className="inline-flex items-center justify-center w-12 h-12 rounded-2xl bg-primary/10 text-primary mb-4">
      ...
      fallback={
        <div className="page-projects space-y-10">
          <div className="text-center py-8 md:py-12">
            ...
            <p className="text-lg text-muted-foreground">{getLabel(config, "loading")}</p>
          </div>
        </div>
      }
    >
