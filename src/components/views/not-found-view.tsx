/**
 * Shared 404 page renderer (locale-aware).
 *
 * Wraps NotFoundContent inside SiteShell so root `not-found.tsx` — which
 * doesn't inherit `(site)/layout.tsx` — still gets the same navbar/footer
 * chrome as an in-group 404.
 */
...
interface NotFoundViewProps {
  locale?: string;
  /** If true, wraps content in SiteShell (used by root not-found.tsx). */
  withShell?: boolean;
}
    ...
    <div className="page-not-found flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
      ...
      <div className="flex flex-wrap justify-center gap-3">
        ...
        <Button variant="outline" size="lg" asChild>
          <Link href={`${prefix}/blog`} className="flex items-center gap-2">
...
export function NotFoundView({ locale, withShell = false }: NotFoundViewProps = {}) {
  if (withShell) {
    return (
